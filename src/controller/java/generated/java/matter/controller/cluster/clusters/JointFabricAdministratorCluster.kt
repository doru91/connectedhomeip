...
class JointFabricAdministratorCluster(private val controller: MatterController, private val endpointId: UShort) {
  class ICACCSRResponse(
    val icaccsr: ByteArray
  )
...
class AttributeListAttribute(
  ...
  suspend fun ICACCSRRequest(timedInvokeTimeout: Duration? = null): ICACCSRResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ICACCSR)) {icaccsr_decoded = tlvReader.getByteArray(tag)}  
  ...
  suspend fun addICAC(ICACValue: ByteArray
      ,timedInvokeTimeout: Duration? = null): ICACResponse {
    val commandId: UInt = 2u

    val tlvWriter = TlvWriter()
    tlvWriter.startStructure(AnonymousTag)

    val TAG_ICAC_VALUE_REQ: Int = 0
  ...
  suspend fun openJointCommissioningWindow(commissioningTimeout: UShort
...
suspend fun readAdministratorFabricIndexAttribute(): UByte {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Administratorfabricindex attribute not found in response" 
    }
  ...
  suspend fun subscribeAdministratorFabricIndexAttribute(
    minInterval: Int,
    maxInterval: Int
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Administratorfabricindex attribute not found in Node State update" 
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(JointFabricAdministratorCluster::class.java.name)
    const val CLUSTER_ID: UInt = 1875u
  }
}
